...
from autotable.storage_model.table import TablePr
...
def update_pr_table(table: Table, title_identifier: str, prs: PaginatedList[PullRequest]) -> Table:
    ...
    for i in prs:
        # 转换为自己的类型
        pr_state: PrType = get_pr_type(i)
        if pr_state is PrType.CLOSED:
            close_number.append(i.number)
            continue

        # 中英符号转换
        title = str_translate(i.title)
        # 截取标题的有用信息
        ...
        # 获取reviews
        pr_reviews: PaginatedList[PullRequestReview] = i.get_reviews()
        ...
        for index in pr_index:
            ...
            # 这里直接处理成 pr 号处理
            pr_table_list: list[TablePr] = [TablePr(pr_state.match_pr_table(), i.number)]
            ...
            # 处理人名
            # 第一位是@位, 第二位是状态位
            people_names: list[str] = [i.user.login]
            people_names.extend(analysis_table_more_people(table_claim_people))
            table_people_names = ""
            for name in people_names:
                table_people_names += f"@{name}"
